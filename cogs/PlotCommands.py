        ...
        if valid_plot(name) and getplotnumber(user,name) > 0:
            ...
            for m,num in plot["machines"].items():
                ...
                if count > 17:
                    count = 0
                    pages.append(em)
                    em = discord.Embed(title = "Plot {}/{}".format(len(pages)+1, description = f"{plot['number']} plots | {getplotnummachines(user,name)} machines | {getplotnumresources(user,name)} resources | {energproducedbyplot(user)[name]} energy produced | {energyconsumedbyplot(user)[name]} energy consumed{'' if doesplothaveenoughenergy(user,name) else ' | More power needed!'}", math.ceil(len(plots)/18)),color=colorInfo())
